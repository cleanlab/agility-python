...
logging.getLogger("agility").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Agility]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Agility(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client
